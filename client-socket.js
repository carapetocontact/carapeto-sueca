...
function enviarJogada(playerIndex, cardIndex) {
  // Copiar a carta ANTES de qualquer update ou reorder
  const cartaOriginal = hands[playerIndex][cardIndex];

  // Segurança
  if (!cartaOriginal) {
    console.error("[SALA] ERRO: cartaOriginal undefined!", { playerIndex, cardIndex, mao: hands[playerIndex] });
    return;
  }

  // Criar um clone para não ser alterado pelo motor
  const cartaReal = { valor: cartaOriginal.valor, naipe: cartaOriginal.naipe };

  debugLogSALA("Enviei jogada:", playerIndex, cartaReal.valor + cartaReal.naipe);
